...
- AI 问答：用户可选择知识分类限定召回范围；系统先让大模型改写用户问题，再通过 MySQL LIKE + 应用层打分召回 top3 切片，最后注入 Prompt 生成回答。
...
3. 如果用户选择了知识分类，后端先限定对应分类下的活动文档，再检索其切片。
4. 后端用 MySQL LIKE 检索 `knowledge_chunk` 的标题和正文，并在 Java 层计算匹配分；技术词优先级高于“区别、流程”等通用意图词。
5. 取 top3 切片和所属知识分类注入 Prompt，要求模型只基于召回资料回答。
6. 如果 `DEEPSEEK_API_KEY` 未配置，接口返回配置提示，并附带当前检索到的切片内容。
