...
class AiServiceTest {
    ...
    void askReturnsConfigurationHintAndTopChunksWhenModelIsUnavailable() {
        ...
        when(documentService.searchForAi(eq(List.of("RAG 的流程是什么？")), eq(3), eq(null)))
    ...
    @Test
    void askPassesSelectedKnowledgeCategoryToRetrieval() {
        KnowledgeDocumentService documentService = mock(KnowledgeDocumentService.class);
        @SuppressWarnings("unchecked")
        ObjectProvider<OpenAiChatModel> provider = mock(ObjectProvider.class);
        when(provider.getIfAvailable()).thenReturn(null);
        when(documentService.searchForAi(eq(List.of("Embedding 和 RAG 的区别？")), eq(3), eq("大模型")))
                .thenReturn(List.of());

        AiService service = new AiService(documentService, provider);

        service.ask("Embedding 和 RAG 的区别？", "大模型");

        verify(documentService).searchForAi(List.of("Embedding 和 RAG 的区别？"), 3, "大模型");
    }
}
